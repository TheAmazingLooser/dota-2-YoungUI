...
YoungUi.ChildIsPlayer = false -- if this is true, no checks happen if its for the local client or not
YoungUi.ChildIsLocal = false
...
function YoungUi:MasterSend(EventName, TableValues)
	print("FireHameEventLocal("..tostring(EventName)..","..tostring(TableValues)..")")
	FireGameEventLocal(EventName,TableValues)
	print("FireHameEvent("..tostring(EventName)..","..tostring(TableValues)..")")
	FireGameEvent(EventName,TableValues)
	return self
end
...
function YoungUi:CreatePanel(Type, ID)
	...
	self:MasterSend("young_ui_create_panel",
	{
		type = Type,
		parent = self.Id,
		id = usedId
...
function YoungUi:_Delete(__LuaDelete)
	...
	if self.Parent then
		MasterSend("young_ui_delete_panel",
		{
			id = self.Id
		})
...
function YoungUi:SetAttribute(AttributeName, AttributeValue)
	self:MasterSend("young_ui_set_attribute",
	{
		id = self.Id,
...
function YoungUi:SetStyleAttribute(AttributeName, AttributeValue)
	self:MasterSend("young_ui_set_style_attribute",
	{
		id = self.Id,
		attribute_name = AttributeName,
		attribute_value = AttributeValue -- if this is nil, the attribute is getting deleted
...
function YoungUi:ApplyClass(ClassName)
	self:MasterSend("young_ui_set_panel_class",{
		id = self.Id,
...
function YoungUi:RemoveClass(ClassName)
	self:MasterSend("young_ui_remove_panel_class",{
		id = self.Id,
		class_name = ClassName or "ERROR_NO_CLASS_NAME"
	})
	return self
